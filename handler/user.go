...
func (h *userHandler) CheckEmailAvaibility(c *gin.Context) {
	...
	if err != nil {
		errorMessgae := gin.H{"error": "Server error"}
		response := helper.APIResponse("Email cheking failed", http.StatusUnprocessableEntity, "error", errorMessgae)
		c.JSON(http.StatusUnprocessableEntity, response)
		return
	}
